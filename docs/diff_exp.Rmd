...
On the virtual machine the full path to the workshop directory is `/home/ubuntu/resources/workshops/ws4-differential-expression/`. Within this directory you will see the `DE/` directory, plus other directories that will be used throughout the workshop. For this section of the workshop you will need access to the gene counts data that you generated in the previous section of the workshop and stored in the `DE/` directory. You should be able to see these folders and files using the Graphical SFTP browser in MobaXTerm.
...
The above code chunk first reads in the samples table, selects only the samples at 0hr OR 1hr, re-factors the timepoint data (otherwise it contains metadata on other timepoints) and finally reverses the order of the table so that it matches the counts table. ii) We also our table of counts that was produced with the `featureCounts` program:
...
Here, we use the `gsub` command to first remove the path to the BAM files from the filename and then use the substring (`substr`) command on the `colnames` of the counts table to just keep characters 1 - 11 (i.e the SRR ID). Using `head` to look at the first few lines of the counts table shows us that our column names are much more readable and can be easily cross-referenced with the samples table.
...
The above code chunk converts our counts to CPM and stores the result in `counts.cpm`. We then create a logical table that stores TRUE values if the count for a gene and sample is above our CPM cutoff of 10.  If the count is any smaller, it is considered to be very low, indicating that the associated gene is not expressed, or we can not accurately detect it in that sample. Next, we use `rowSums` to count the number of samples that show a CPM greater than our cutoff, a requirement for expression in two or more libraries is used as each group contains two replicates. This ensures that a gene will be retained if it is only expressed in one group. Finally, we subset our counts table to create `counts.keep` that now stores our filtered counts data. Using `dim` we can see that we now have 5,697 genes in our dataset. 

### Create a DGEList object

Next we’ll create a DGEList object, an object used by edgeR to store count data. It has a number of slots for storing various parameters about the data.
...
As a quick check of our results, we can plot a `barplot` of the CPM normalised counts for one of our significantly differentially expressed genes. We choose gene `CNB3_005342` with a `logFC` of -8.82:

```{r, eval=FALSE}
pdf('diff_exp_barplot.pdf')
barplot(counts.cpm['CNB3_005342', ], xaxt='n', ann=FALSE, xlab='Samples')
dev.off()
```
...
As before the code will produce a PDF of the bar plot (`diff_exp_barplot.pdf`) in the workshop base directory. You can use MobaXTerm to transfer the file to your local machine and view it. We can see that in samples 1-5 (columns 1-5 on the x axis) and 11-15 (columns 11-15 on the x axis), the 0hr timepoint samples, there are high CPM values indicating high expression and very little expression in samples 6-10 and 16-20 (the 1hr timepoint samples). This confirms our result of significant down-regulation for `CNB3_005342` at 1hr post infection.
