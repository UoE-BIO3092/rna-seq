...
Text about what we will do in this section
...
To build an index we need a fasta file that stores the *C. gattii* genome. You will find some reference sequences for the pathogen in `C.gattii_ref/`. The reference sequence is stored in file `Cryptococcus_gattii.fa`. Let's start by loading the Rsubread package and buildiing our reference.

```{r}
library(Rsubread)
buildindex(basename="C.gattii_ref/Cryptococcus_gattii", reference="C.gattii_ref/Cryptococcus_gattii.fa")
```
...
fc = featureCounts(bam.files, annot.ex="C.gattii_ref/Cryptococcus_gattii.gff3", isGTFAnnotationFile=TRUE, GTF.featureType = "gene", GTF.attrType = "ID", isPairedEnd=TRUE, nthreads=4)
```
...
which tells us that we have 6,472 rows (or genes) and 20 columns (or samples - 5 strains * 2 timepoints * 2 replicates).
...
In the next stage of our workshop we will carry out some differential expression analysis. To go back to the index page click [here](https://uoe-bio3092.github.io/rna-seq/03_workshop.html).
