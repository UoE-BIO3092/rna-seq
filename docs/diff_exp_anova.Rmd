...
For this section of the workshop you will need access to the gene counts data that you generated in the previous section of the workshop and stored in the `DE/` directory. On the virtual machine the full path to the workshop directory is `/home/ubuntu/resources/workshops/ws4-differential-expression/`. Within this directory you will see the `DE/` directory, plus other directories that will be used throughout the workshop.
...
We use the levels information to assign colours to each of the strains. Then we call `plotMDS` to visualise the variation in the data. We can see clearly that most of our strains do seem to cluster together. We can notice that strains R265 and ENV152 are very similar. Also our replicates of WM276 appear to be quite distinct compared to the other strains. We may see this pattern because of issues with data quality (we know that we have few mapping and assigned reads in these data) or because of differences between the *C. gattii* strains in our experiment. It is a good result that we can distinguish between our strains.
