...
On the virtual machine the full path to the workshop directory is `/home/ubuntu/resources/workshops/ws4-differential-expression/`. Within this directory you will see the `DE/` and `C.gattii_ref/` directories, plus other directories that will be used throughout the workshop. For this section of the workshop you will need access to the gene counts data that you generated in the previous section of the workshop and stored in the `DE/` directory as well as *C. gattii* annotation  data stored in `C.gattii_ref/`. 
...
Then we use the `plot_anno_scores` function to visualise our 4 significantly enriched GO terms. On the virtual machine R will create a new PDF called `Rplots.pdf` that stores this visualisation. You can download and view this file from MobaXTerm. In the resulting plots the pie charts show the amounts of candidate and background genes that are annotated to the GO-categories and the root nodes (candidate genes in the colour of the corresponding root node). The top panel shows the odds-ratio and 95%-Confidence Interval from a Fisher’s exact test (two-sided) comparing the GO-categories with their root nodes.
...
* Performed functional pathway analysis using the Gene Ontology
